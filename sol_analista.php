...
if($fecharsol==1) {
    ...
    if($_POST['limite']==''){$limite=100;}else{$limite = $_POST['limite'];}
    ...
    WHERE
        ...
        solicitacao.usr_cod = '$analista' and
    ...
    $dados1 = mysql_fetch_array($consulta1);
        ...
        {
            if($ano!=9999 and $ano!='' and $mes!=99 and $mes!='' and $datafecha!='9999-99-99 99:99:99' and $datafecha!='' and $analista!='')
            {
            //ADICIONAR
            $sql2 = "
    ...
    ?>
    <br>
    <form action='sol_analista.php' method='POST'>
    <table>
        <tr>
            <td>
                <select name='ano'>
            ...
            <td><input type='text' style="text-align:center" maxlength='19' size='20' name='datafecha' <?php if($datafecha != ''){echo "value='$datafecha'";}else{echo "value='9999-99-99 18:00:00'";} ?>></td>
            <td>
                <select name='analista'>
                    <option value=''>Selecione o Analista</option>
                    <?php
                    $sql6 = "SELECT usr_cod,usr_nome FROM usuario WHERE emp_cod=1 ORDER BY usr_nome ASC";
                    $consulta6 = mysql_query($sql6, $con) or die (mysql_error());
                    while ($dados6 = mysql_fetch_array($consulta6)) {
                    ?>
                    <option value='<?php print_r($dados6['usr_cod'])?>' <?php if($dados6['usr_cod'] == $analista){echo "SELECTED";}?>><?php echo utf8_encode($dados6['usr_nome']);?>
                    <?php }	?>
                </select>
            </td>
            <td><input type='text' style="text-align:center" maxlength='6' size='5' name='limite' <?php if($limite != ''){echo "value='$limite'";}else{echo "value='100'";} ?>></td>
        ...
        <tr>
            <td>
                <form action='sol_analista.php' method='POST'>
                    ...
                    <input type='hidden' name='analista' value='<?php print_r($analista)?>'>
                    <input type='submit' name='mudar' value='Fechar'>
                </form>
            </td>
            <td><form action='sol_analista.php'><input type='submit' value='Limpar'></form></td>
        </tr>
    </table>
    <?php
    if ($_POST['mudar']=='Fechar'){
        ...
        <tr>
            <td valign='top'>
                ...
                <table style="border-collapse: collapse" cellspacing='0' cellpadding='0' border='1' width='450'>
                    <tr>
                        <td bgcolor='Green' colspan='5' align='center'><b>Solicitações Abertas</b></td>
                    </tr>
                    <tr>
                        <td bgcolor='Green' colspan='5' align='center'><font color='white'>Obs.: Todas as Solicitações <u>ABERTAS</u> pelo 'Analista', no 'Ano' e 'Mês' espeficicado na data da 'ùltima Alteração' e no Status<BR>'LIBERADO P/ IMPLANTAÇÃO'.</font></td>
                    </tr>
                    <tr bgcolor='#ffffff'>
                ...
                <?php }
                if($cont==1){echo "<tr bgcolor='#ffffff'><td colspan='5' align='center'>Não há solicitação aberta!!!</td></tr>";}
                ?>
                </table>
            </td>
            <td valign='top'>
                ...
                <table style="border-collapse: collapse" cellspacing='0' cellpadding='0' border='1' width='450'>
                    <tr>
                        <td bgcolor='red' colspan='5' align='center'><b>Solicitações Fechadas</b></td>
                    </tr>
                    <tr>
                        <td bgcolor='red' colspan='5' align='center'><font color='white'>Obs.: Todas as Solicitações <u>FECHADAS</u> pelo 'Analista', no 'Ano' e 'Mês' espeficicado na data da 'ùltima Alteração' e em<br>QUALQUER Status.</font></td>
                    </tr>
                    <tr bgcolor='#ffffff'>
                        <td align='center'><b>N</b></td>
                        <td align='center'><b>Codigo</b></td>
                ...
                <?php }
                if($cont==1){echo "<tr bgcolor='#ffffff'><td colspan='5' align='center'>Não há solicitação fechada!!!</td></tr>";}
                ?>
                </table>
            </td>
        </tr>
